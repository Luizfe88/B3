...
def is_valid_dataframe(obj: Union[pd.DataFrame, np.ndarray], min_rows: int = 1) -> bool:
    """
    Helper robusto para validar entradas.
    Aceita DataFrame (não vazio, linhas >= min_rows) ou ndarray (tamanho >= min_rows).
    """
    try:
        if isinstance(obj, pd.DataFrame):
            return (obj is not None) and (not obj.empty) and (len(obj) >= min_rows)
        if isinstance(obj, np.ndarray):
            return (obj is not None) and (obj.size >= min_rows)
        # Lista/Series como fallback
        if hasattr(obj, "__len__"):
            return len(obj) >= min_rows
        return False
    except Exception:
        return False
...
class PerformanceMetrics:
    ...
    def __str__(self) -> str:
        ...
        sharpe_emoji = (
            ...
            else "🟡" if self.sharpe_ratio > 1.0 else "🔴"
        )
        wr_emoji = (
            "🟢" if self.win_rate > 0.55 else "🟡" if self.win_rate > 0.45 else "🔴"
        )
...
def calculate_returns(
    equity_curve: np.ndarray, config: MetricsConfig = DEFAULT_CONFIG
) -> Dict[str, float]:
    """
    ...
    if not is_valid_dataframe(equity_curve, min_rows=2):
        return {
            "total_return": 0.0,
...
def calculate_drawdown_metrics(
    ...
    return {
        ...
        "recovery_time_days": (
            float(recovery_time_days) if recovery_time_days is not None else None
        ),
    }
...
def calculate_sharpe_ratio(
    ...
    if not is_valid_dataframe(returns, min_rows=2):
        return 0.0
    ...
    # ✅ Proteção contra super-anualização com poucos dados
    if len(returns) < max(50, config.bars_per_day * 2):
        sharpe = min(sharpe, 9.99)
...
def calculate_var_cvar(
    ...
    if not is_valid_dataframe(returns, min_rows=10):
...
def calculate_streaks(trades_df: pd.DataFrame) -> Dict[str, Union[int, str]]:
    ...
    # Agrupa por streak
    streaks = trades_df.groupby(streak_ids).agg({"pnl_money": ["sum", "count"]})
...
def calculate_exposure_metrics(
    trades_df: pd.DataFrame, total_bars: int, config: MetricsConfig = DEFAULT_CONFIG
) -> Dict[str, float]:
    """
...
def calculate_all_metrics(
    ...
    if returns_metrics["total_return"] <= 0 or trading_metrics["profit_factor"] < 1.0:
...
if __name__ == "__main__":
    ...
    trades_data = {
        ...
        "exit_time": pd.date_range("2025-01-01", periods=n_trades, freq="H")
