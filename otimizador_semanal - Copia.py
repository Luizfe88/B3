...
def check_liquidity_dynamic(sym: str, ibov_df: pd.DataFrame = None) -> dict:
    ...
    try:
        ...
        reason = (
            ""
            if is_liquid
            else f"Baixa Liquidez (< R$ {MIN_FINANCEIRO/1_000_000:.1f}M)"
        )
        ...
        if rates_m15 is not None and len(rates_m15) > 50:
            ...
            avg_atr_pct = np.mean(atr_vals[-50:]) / np.mean(
                df_m15["close"].values[-50:]
            )
...
def extract_features(indicators, symbol):
    ...
    # Novo: Sentiment X
    sentiment = x_semantic_search(
        f"sentimento {symbol} B3", limit=5
    )  # Tool call simulado
...
def load_data_with_retry(
    symbol: str, bars: int, timeframe=None, max_retries: int = 3
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if (
                rates is not None and len(rates) >= 500
            ):  # Exigimos pelo menos 500 barras para qualidade
                ...
                logger.warning(
                    f"⚠️ {symbol}: Poucos dados ({len(rates) if rates is not None else 0}) - Tentando backfill..."
                )
    ...
    for i in range(1, n):
        ...
        if state == 1:
            ...
            if high[i] >= target_price:
                ...
                cost = (position * transaction_cost_pct) + (
                    (position + gross_profit) * transaction_cost_pct
                )
...
def run_monte_carlo_stress(
    ...
    for sim_idx in range(n_simulations):
        ...
        if np.random.random() < 0.30 and all(
            len(v) > 10 for v in stress_scenarios.values()
        ):
            scenario = np.random.choice(
                list(scenario_weights.keys()), p=list(scenario_weights.values())
            )
            sim_returns = np.random.choice(
                stress_scenarios[scenario], size=n_bars, replace=True
            )
        else:
...
def run_optimizer():
    ...
    with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        results = list(
            tqdm(
                executor.map(process_symbol_wrapper, tasks),
                total=len(tasks),
                desc="Otimizando",
            )
        )
    ...
    final_selection_all = sorted(
        optimized_list,
        key=lambda x: (
            float(x.get("res", {}).get("test_metrics", {}).get("profit_factor", 0.0))
            * 0.7
            - float(x.get("res", {}).get("test_metrics", {}).get("max_drawdown", 1.0))
            * 0.3
        ),
        reverse=True,
    )[:20]
