...
class LogAnalyzer:
    ...
    def analyze_single_day(self, date_str: str = None) -> Dict:
        ...
        strategy_executed = Counter(
            a["strategy"] for a in analyses if a["executed"] and a["strategy"] != "N/A"
        )
    ...
    def analyze_date_range(self, start_date: str, end_date: str) -> Dict:
        ...
        return {
            ...
            "rejection_reasons": Counter(
                a["reason"] for a in all_analyses if not a["executed"]
            ),
            "avg_score_all": statistics.mean(a["score"] for a in all_analyses),
            "avg_score_executed": (
                statistics.mean(a["score"] for a in all_analyses if a["executed"])
                if executed > 0
                else 0
            ),
        }
    ...
    def generate_report(self, stats: Dict) -> str:
        ...
        lines.append(
            f"✅ Executadas: {stats['executed']} ({stats['execution_rate']:.1f}%)"
        )
        lines.append(
            f"❌ Rejeitadas: {stats['rejected']} ({100-stats['execution_rate']:.1f}%)"
        )
        lines.append("")
        ...
        for symbol, count in stats["symbols"].most_common(10):
            ...
            lines.append(
                f"{symbol:10s} | {count:3d} análises | {executed_count:2d} executadas ({exec_rate:.0f}%)"
            )
        lines.append("")
        ...
        if stats.get("strategies"):
            ...
            for strategy, count in stats["strategies"].most_common():
                ...
                exec_rate = (executed_count / count * 100) if count > 0 else 0
                lines.append(
                    f"{strategy:10s} | {count:3d} sinais | {executed_count:2d} executados ({exec_rate:.0f}%)"
                )
            lines.append("")
...
def interactive_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            date = input(
                "\n📅 Data do relatório (YYYY-MM-DD) ou ENTER para hoje: "
            ).strip()
            if not date:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "today":
            ...
            print(
                "  python log_analyzer.py date 2026-01-05    # Analisa data específica"
            )
            print(
                "  python log_analyzer.py range 2026-01-01 2026-01-07    # Analisa período"
            )
    else:
        # Modo interativo
        interactive_menu()
