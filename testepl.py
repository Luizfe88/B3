...
def analisar_log_trades(filename):
    ...
    if estatisticas["pnl_zerado_saida"] > 0:
        print(
            f"🚨 PROBLEMA CRÍTICO: {estatisticas['pnl_zerado_saida']} SAÍDAS com P&L ZERADO!"
        )
        print(
            "   Isso indica que close_position() não está calculando P&L corretamente\n"
        )
...
def main():
    ...
    logs_encontrados = [
        f for f in os.listdir(".") if f.startswith("trades_log_") and f.endswith(".txt")
    ]
