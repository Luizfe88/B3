...
print("[DEBUG] Importando threading.RLock...", flush=True)
from threading import RLock

print("[DEBUG] threading.RLock importado.", flush=True)
print("[DEBUG] Importando config...", flush=True)
import config as config

print("[DEBUG] config importado.", flush=True)
try:
    print("[DEBUG] Importando pytz...", flush=True)
    import pytz

    print("[DEBUG] pytz importado.", flush=True)
except Exception:
    pytz = None
    print("[DEBUG] pytz indisponível.", flush=True)
...
# ✅ Exportações
__all__ = ["NewsCalendarMT5", "news_calendar", "apply_blackout"]
