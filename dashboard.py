...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("dashboard_institutional.log", encoding="utf-8")],
)
    ...
    try:
        ...
        if acc:
            return {
                ...
                "margin_level": (
                    (acc.equity / acc.margin * 100) if acc.margin > 0 else 0
                ),
                "profit": acc.profit,
                "login": acc.login,
                "server": acc.server,
                ...
                if line.strip() and not line.startswith("-"):
                    ...
                    if len(parts) >= 8:
                        try:
                            ...
                            pnl_str = (
                                parts[6].strip().replace("R$", "").replace(",", "")
                            )
...
def calculate_advanced_metrics(trades_df):
    ...
    total_days = (
        (trades_df["Timestamp"].max() - trades_df["Timestamp"].min()) / 86400
        if len(trades_df) > 1
        else 1
    )
    ...
    try:
        ...
        if symbols is None:
            ...
            symbols = (
                ...
                if hasattr(config, "ELITE_SYMBOLS")
                else ["PETR4", "VALE3", "ITUB4", "BBDC4"]
            )
...
# ===========================
st.markdown(
    '<div class="main-header">🏛️ XP3 INSTITUTIONAL DASHBOARD v6.0</div>',
    unsafe_allow_html=True,
)
...
with st.sidebar:
    st.image(
        "https://via.placeholder.com/200x60/0f3460/FFFFFF?text=XP3+PRO", width="stretch"
    )
...
with col_title:
    st.markdown(
        '<div class="section-header">📊 Painel de Controle - Visão Geral</div>',
        unsafe_allow_html=True,
    )
...
with col_status:
    try:
        ...
        regime_color = (
            "#00b894"
            if "BULL" in regime
            else "#d63031" if "BEAR" in regime else "#fdcb6e"
        )
...
with kpi1:
    ...
    st.markdown(
        '<div class="kpi-label">💰 Patrimônio Líquido</div>', unsafe_allow_html=True
    )
    st.markdown(
        f'<div class="kpi-value">R$ {acc["equity"]:,.2f}</div>', unsafe_allow_html=True
    )
    pnl_day = acc["profit"]
    delta_color = "🟢" if pnl_day >= 0 else "🔴"
    st.markdown(
        f'<div style="font-size: 0.9rem;color: #000000">{delta_color} R$ {pnl_day:+,.2f} hoje</div>',
        unsafe_allow_html=True,
    )
...
with kpi2:
    st.markdown('<div class="kpi-card">', unsafe_allow_html=True)
    st.markdown(
        '<div class="kpi-label">📈 Retorno Diário</div>', unsafe_allow_html=True
    )
    ...
    st.markdown(
        f'<div style="font-size: 0.9rem;color: #000000">Base: R$ {acc["balance"]:,.2f}</div>',
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
with kpi3:
    st.markdown('<div class="kpi-card">', unsafe_allow_html=True)
    st.markdown(
        '<div class="kpi-label">🎯 Posições Ativas</div>', unsafe_allow_html=True
    )
    st.markdown(
        f'<div class="kpi-value">{len(positions_df)}</div>', unsafe_allow_html=True
    )
    max_pos = getattr(config, "MAX_SYMBOLS", 10)
    st.markdown(
        f'<div style="font-size: 0.9rem;color: #000000">Limite: {max_pos} posições</div>',
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
with kpi4:
    st.markdown('<div class="kpi-card">', unsafe_allow_html=True)
    st.markdown(
        '<div class="kpi-label">🛡️ Nível de Margem</div>', unsafe_allow_html=True
    )
    st.markdown(
        f'<div class="kpi-value">{acc["margin_level"]:.1f}%</div>',
        unsafe_allow_html=True,
    )
    margin_status = "Seguro" if acc["margin_level"] > 200 else "Atenção"
    st.markdown(
        f'<div style="font-size: 0.9rem;color: #000000">{margin_status}</div>',
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
with kpi5:
    ...
    st.markdown(
        f'<div class="kpi-value">R$ {lucro_flutuante:+,.2f}</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<div style="font-size: 0.9rem;color: #000000">{len(positions_df)} operações abertas</div>',
        unsafe_allow_html=True,
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
with ia_col1:
    st.markdown(
        '<div class="section-header">🤖 Narrativa de Mercado (IA)</div>',
        unsafe_allow_html=True,
    )
    ...
    narrative = llm_narrative.generate_market_narrative(
        active_sym, dummy_ind, sentiment
    )

    st.info(narrative)
    st.markdown(
        f"<div style='font-size: 1.0rem;color: #000000'>Sentimento consolidado ({active_sym}): {sentiment:+.2f}</div>",
        unsafe_allow_html=True,
    )

with ia_col2:
    st.markdown(
        '<div class="section-header">💎 Elite Symbols Checklist</div>',
        unsafe_allow_html=True,
    )
    ...
    if elite_list:
        ...
        symbols_to_show = (
            list(elite_list.keys()) if isinstance(elite_list, dict) else elite_list
        )
        st.write(", ".join(symbols_to_show[:8]) + "...")
        # Barra de progresso de cobertura
        coverage = (
            min(100, (len(positions_df) / len(elite_list)) * 100) if elite_list else 0
        )
...
st.markdown("---")
st.markdown(
    '<div class="section-header">📈 Métricas Avançadas de Performance</div>',
    unsafe_allow_html=True,
)
...
if metrics:
    ...
    with col9:
        ...
        st.metric(
            "⚖️ Risk/Reward", f"{rr_ratio:.2f}", help="Relação ganho médio / perda média"
        )
    ...
    with col12:
        recovery_factor = (
            abs(metrics["total_pnl"] / metrics["max_drawdown"])
...
st.markdown("---")
st.markdown(
    '<div class="section-header">📈 Curva de Equity e Análise de Drawdown</div>',
    unsafe_allow_html=True,
)
...
if not historical_trades.empty and "P&L" in historical_trades.columns:
    ...
    if not closed_trades.empty:
        ...
        closed_trades["Drawdown"] = (
            closed_trades["P&L Acumulado"] - closed_trades["Máximo Acumulado"]
        )
...
st.markdown("---")
st.markdown(
    '<div class="section-header">🔍 Análise de Oportunidades Rejeitadas</div>',
    unsafe_allow_html=True,
)
...
def load_rejections_data():
    ...
    # Prefer logs/analysis com buckets horários
    try:
        if analysis_dir.exists():
            files_to_read = sorted(analysis_dir.glob(f"analysis_{today}_*.txt"))
        # Fallback antigo: analysis_logs/analysis_log_<date>.txt
        if not files_to_read:
            legacy = Path(f"analysis_logs/analysis_log_{today}.txt")
            if legacy.exists():
                files_to_read = [legacy]
    except Exception as e:
        logger.error(f"Erro ao listar arquivos de análise: {e}")
        files_to_read = []
    ...
    try:
        logger.info(f"Carregando {len(files_to_read)} arquivo(s) de análise de {today}")
        for log_file in files_to_read:
            ...
            for block in blocks:
                ...
                for line in lines:
                    ...
                    if "Sinal:" in line:
                        ...
                        try:
                            ...
                            entry["Signal"] = parts[0]

                    if "Filtros de Setup:" in line:
                        # Extrai percentuais do progresso quando possível
                        try:
                ...
                # Inclui todas análises não executadas (qualquer tipo de rejeição/bloqueio)
                if entry and "Status" in entry and ("EXECUTADA" not in entry["Status"]):
                    data.append(entry)

        logger.info(f"Entradas de rejeição carregadas: {len(data)}")
    except Exception as e:
...
if not rejections_df.empty:
    ...
    with r_col1:
        ...
        st.dataframe(
            rejections_df[
                ["Time", "Symbol", "Signal", "Score", "Reason", "RSI", "Volume"]
            ],
            width="stretch",
            height=300,
        )
    ...
    with r_col2:
        ...
        if reasons_stats:
            reasons_df = pd.DataFrame(
                list(reasons_stats.items()), columns=["Motivo", "Contagem"]
...
st.markdown("---")
st.markdown(
    '<div class="section-header">🎯 Performance por Ativo</div>', unsafe_allow_html=True
)

if not historical_trades.empty and "Símbolo" in historical_trades.columns:
    # Análise por ativo
    symbol_analysis = (
        historical_trades[historical_trades["Tipo"] == "SAÍDA"]
        .groupby("Símbolo")
        .agg(
            {
                "P&L": ["sum", "mean", "count"],
                "Lado": lambda x: (
                    (x == "COMPRA").sum() / len(x) * 100 if len(x) > 0 else 0
                ),
            }
        )
        .round(2)
    ...
    with col_sym1:
        ...
        colors = [
            "#00b894" if x > 0 else "#d63031" for x in symbol_analysis["P&L Total"]
        ]
...
st.markdown("---")
st.markdown(
    '<div class="section-header">🔥 Heatmap & Análise Temporal</div>',
    unsafe_allow_html=True,
)
...
if not historical_trades.empty and "Hora" in historical_trades.columns:
    try:
        ...
        if not df_hm.empty:
            ...
            heatmap_data = (
                df_hm.groupby(["Símbolo", "Hour"])
                .agg({"P&L": lambda x: (x > 0).sum() / len(x) * 100})  # Win Rate
                .reset_index()
            )
            ...
            heatmap_matrix = heatmap_data.pivot(
                index="Símbolo", columns="Hour", values="Win Rate"
            )
...
st.markdown("---")
st.markdown(
    '<div class="section-header">🔮 Simulador de Cenários Futuros (Monte Carlo)</div>',
    unsafe_allow_html=True,
)
...
if metrics:
    ...
    with c1:
        sim_balance = st.number_input(
            "Capital Inicial", value=float(acc["equity"]), step=1000.0
        )
    with c2:
    ...
    if st.button("🚀 Executar Simulação Preditiva", width="stretch"):
        try:
            ...
            if hist_win_rate == 0:
                ...
                avg_loss = 100
                st.warning(
                    "Histórico insuficiente. Usando parâmetros padrão (WR 50%, RR 1:1)"
                )
            ...
            prob_profit = (
                sum(1 for v in final_values if v > sim_balance) / scenarios * 100
            )

            sc1, sc2, sc3 = st.columns(3)
            sc1.metric(
                "Média Final",
                f"R$ {np.mean(final_values):,.2f}",
                delta=f"{(np.mean(final_values)/sim_balance - 1)*100:.1f}%",
            )
...
st.markdown("---")
st.markdown(
    '<div class="section-header">📋 Posições Abertas - Gestão Ativa</div>',
    unsafe_allow_html=True,
)
...
if not positions_df.empty:
    ...
    total_exposure = (
        positions_display["Volume"] * positions_display["Preço Atual"]
    ).sum()
    positions_display["Exposição %"] = (
        (positions_display["Volume"] * positions_display["Preço Atual"])
        / total_exposure
        * 100
    )
    ...
    def get_close_gap_status(row):
        ...
        close_all_by = (
            getattr(
                config, "FRIDAY_CLOSE_ALL_BY", getattr(config, "CLOSE_ALL_BY", None)
            )
            if now.weekday() == 4
            else getattr(config, "CLOSE_ALL_BY", None)
        )
        ...
        day_close_txt = (
            f"faltam {minutes_to_close:.0f}m p/ day close"
            if minutes_to_close is not None
            else "day close N/A"
        )
        return f"{time_stop_txt}; {day_close_txt}"

    positions_display["Fechamento"] = positions_display.apply(
        get_close_gap_status, axis=1
    )
    ...
    try:
        ...
        st.markdown(
            f'<a href="{href}" download="posicoes_abertas_{datetime.now().strftime("%Y%m%d_%H%M")}.csv">🔗 Download CSV (compatível)</a>',
            unsafe_allow_html=True,
        )
    except Exception as e:
...
st.markdown("---")
st.markdown(
    '<div class="section-header">🧠 Análise Psicológica - Disciplina e Controle Emocional</div>',
    unsafe_allow_html=True,
)
...
if metrics and metrics.get("total_trades", 0) > 5:
    ...
    with col_psych1:
        ...
        if metrics["loss_streak"] > 3:
            st.warning(
                "⚠️ **Alerta:** Sequência de perdas elevada. Considere pausar e revisar estratégia."
            )
    ...
    with col_psych2:
        ...
        if not historical_trades.empty:
            ...
            if len(closed) > 1:
                ...
                consistency_score = (
                    100 - min((std_dev / abs(closed["P&L"].mean()) * 100), 100)
                    if closed["P&L"].mean() != 0
                    else 0
                )
    ...
    st.info(
        "🧠 Execute mais operações para análise psicológica detalhada (mínimo: 5 trades)"
    )
...
st.markdown("---")
st.markdown(
    '<div class="section-header">🏆 Comparação com Benchmarks</div>',
    unsafe_allow_html=True,
)
...
with col_bench1:
    ...
    if metrics:
        period_return = (
            (metrics["total_pnl"] / acc["balance"] * 100) if acc["balance"] > 0 else 0
        )
...
st.markdown("---")
st.markdown(
    '<div class="section-header">📈 Dados de Mercado em Tempo Real</div>',
    unsafe_allow_html=True,
)
...
if not real_time_df.empty:
    ...
    styled_real_time = real_time_df.style.format(
        {"Bid": "{:.2f}", "Ask": "{:.2f}", "Último": "{:.2f}", "Volume": "{:,.0f}"}
    )
...
st.markdown("---")
st.markdown(
    '<div class="section-header">📓 Journal de Trading - Registre suas Observações</div>',
    unsafe_allow_html=True,
)
...
with st.form(key="journal_form"):
    ...
    emotion = st.selectbox(
        "Estado emocional", ["Confiante", "Ansioso", "Frustrado", "Neutro", "Outro"]
    )
...
st.markdown("---")
st.markdown(
    '<div class="section-header">📑 Relatórios Exportáveis</div>',
    unsafe_allow_html=True,
)
...
with col_rep2:
    if st.button("📊 Exportar Dados para Excel"):
        ...
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            historical_trades.to_excel(
                writer, sheet_name="Histórico Trades", index=False
            )
        ...
        try:
            ...
            st.markdown(
                f'<a href="{href}" download="trading_report_{datetime.now().strftime("%Y%m%d")}.xlsx">🔗 Download Excel (compatível)</a>',
                unsafe_allow_html=True,
            )
        except Exception as e:
            st.warning(f"Falha ao preparar link alternativo de download: {e}")
...
def diagnosticar_ativo(symbol):
        ...
        checks.append(
            {
                ...
                "Meta": "Presente",
                "Ok": False,
                "Obs": f"Ativo '{symbol}' não encontrado",
            }
        )
        return checks
    ...
    try:
        ...
        if not params:
            if "WIN" in symbol:
                params = config.ELITE_SYMBOLS.get("WIN$N", {})
            elif "WDO" in symbol:
                params = config.ELITE_SYMBOLS.get("WDO$N", {})
        ...
        rsi_msg = "Neutro"
        if rsi_val < rsi_low:
            rsi_msg = "Sobrevendido (Pode comprar)"
        elif rsi_val > rsi_high:
            rsi_msg = "Sobrecomprado (Pode vender)"
...
st.markdown("---")
st.markdown(
    '<div class="section-header">🔍 Raio-X de Execução (Log Diário - Why no trade?)</div>',
    unsafe_allow_html=True,
)

with st.expander(
    "📂 Abrir Detalhes de Análise de Sinais (Logs Reais do Dia)", expanded=True
):
    ...
    if not df_logs.empty:
        ...
        with c1:
            ...
            sel_symbol = st.selectbox(
                "🎯 Filtrar Ativo", all_symbols, key="sel_sym_log"
            )
        with c2:
            all_statuses = ["Todos"] + list(df_logs["Status"].unique())
            sel_status = st.selectbox(
                "🚦 Filtrar Status", all_statuses, key="sel_stat_log"
            )
        ...
        st.info(
            "📭 Nenhum registro de análise encontrado hoje nos logs. O bot está gerando arquivos em `analysis_logs/`?"
        )

st.markdown("---")
st.markdown(
    '<div class="section-header">🔎 Raio X do Porquê Não Comprou</div>',
    unsafe_allow_html=True,
)
            ...
            try:
                ...
                if df is None or len(df) <= 50:
                    try:
                        ...
                        rates = (
                            mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_M15, 0, 300)
                            or []
                        )
                ...
                if df is not None and len(df) > 50:
                    ...
                    try:
                        ...
                        if vwap is None and {"tick_volume", "close"}.issubset(
                            df.columns
                        ):
                            v = df["tick_volume"]
                            p = df["close"]
...
def _compute_criteria(symbol, side):
    ...
    ema_trend = (
        "UP" if (emaf is not None and emas is not None and emaf > emas) else "DOWN"
    )
    vwap = ind.get("vwap")
    ...
    tech.append(
        (
            "Tendência EMA",
            (side == "BUY" and ema_trend == "UP")
            or (side == "SELL" and ema_trend == "DOWN"),
            ema_trend,
            side,
            "Alinhamento da tendência entre EMAs",
        )
    )
    if rsi is not None:
        tech.append(
            (
                "RSI não esticado",
                (side == "BUY" and rsi <= 70) or (side == "SELL" and rsi >= 30),
                rsi,
                "BUY<=70 | SELL>=30",
                "Prevenção de exaustão de preço",
            )
        )
    if adx is not None:
        tech.append(
            ("ADX mínimo", adx >= 20, adx, ">=20", "Força de tendência suficiente")
        )
    if vwap is not None:
        price_ok = False
        try:
            ...
            if t:
                ...
                price_ok = (side == "BUY" and px > vwap) or (
                    side == "SELL" and px < vwap
                )
                tech.append(
                    (
                        "VWAP alinhado",
                        price_ok,
                        px,
                        f"{'>' if side=='BUY' else '<'} {vwap:.2f}",
                        "Confirmação de direção pelo preço vs VWAP",
                    )
                )
        except Exception:
            pass
    if vol_ratio is not None:
        tech.append(
            (
                ...
                vol_ratio,
                f">={min_vol}",
                "Intensidade de volume intradiário",
            )
        )
    tech.append(
        (
    ...
    try:
        ...
        if df_h1 is not None and len(df_h1) > 50:
            ...
            macro_ok = (side == "BUY" and emaf_h1 > emas_h1) or (
                side == "SELL" and emaf_h1 < emas_h1
            )
            macro.append(
                (
                    "H1 alinhado",
                    macro_ok,
                    "UP" if emaf_h1 > emas_h1 else "DOWN",
                    side,
            ...
            macro.append(
                (
                    "ADX H1 override",
                    adx_h1 >= override_adx if not macro_ok else True,
                    adx_h1,
                    f">={override_adx}",
                    "Override de macro se ADX forte",
                )
            )
    except Exception:
        pass
    ...
    try:
        ...
        liqui.append(
            (
                "Liquidez projetada",
                bool(ok_liq),
                "OK" if ok_liq else "Baixa",
                ">= limiar",
                "Capacidade de execução sem impacto excessivo",
            )
        )
    except Exception:
        liqui.append(
            ("Liquidez projetada", False, "N/A", ">= limiar", "Dados indisponíveis")
        )
    oper = []
    try:
        no_entry = (
            getattr(
                config,
                "FRIDAY_NO_ENTRY_AFTER",
                getattr(config, "NO_ENTRY_AFTER", "17:35"),
            )
            if now.weekday() == 4
            else getattr(config, "NO_ENTRY_AFTER", "17:35")
        )
        ok_time = True
        try:
        ...
        oper.append(
            (
                ...
                now.strftime("%H:%M"),
                f"< {no_entry}",
                "Restrições de entrada por horário",
            )
        )
    except Exception:
        pass
        ...
        if BOT_CONNECTED and "daily_trades_per_symbol" in globals():
            ...
            oper.append(
                (
                    ...
                    count,
                    "< 4",
                    "Controle de tentativas por ativo",
                )
            )
    except Exception:
        pass
    ...
    try:
        ...
        noticias.append(
            (
                "Sem blackout de notícias",
                not is_blackout,
                "OK" if not is_blackout else reason,
                "—",
                "Filtro de eventos macro relevantes",
            )
        )
    except Exception:
        noticias.append(
            ("Sem blackout de notícias", True, "N/A", "—", "Dados indisponíveis")
        )
    return {
        "Tecnica": tech,
        "Macro": macro,
...
if symbols_list:
    ...
    with g1:
        ...
        for name, ok, value, threshold, tip in crit["Tecnica"]:
            st.markdown(
                _badge_row(name, ok, value, threshold, tip), unsafe_allow_html=True
            )
        st.markdown("### 🌎 Macro")
...
with st.expander("Clique aqui para forçar um check técnico agora"):
    ...
    lista_ativos = getattr(
        config, "ELITE_SYMBOLS", ["WIN$N", "WDO$N", "PETR4", "VALE3"]
    )
    if isinstance(lista_ativos, dict):
        lista_ativos = list(lista_ativos.keys())
    ...
    for ativo in lista_ativos:
        with cols_diag[col_counter % 3]:
            ...
            lado = (
                "BUY"
                if (emaf is not None and emas is not None and emaf > emas)
                else "SELL"
            )
            now = datetime.now()
            hour = now.hour
            ...
            rsi_html = _badge_row(
                "RSI (14)", ok_rsi, rsi_val, "30-70", "Equilíbrio de momentum"
            )
...
st.markdown("---")
st.markdown(
    '<div class="section-header">📋 Quadro de Tarefas</div>', unsafe_allow_html=True
)
...
def _seed_tasks():
    return [
        {
            "id": "tk_liquidez",
            "titulo": "Checar liquidez Top 15",
            "status": "pending",
            "categoria": "Operacional",
            "desc": "Validar volume projetado e profundidade do book",
        },
        {
            "id": "tk_ml",
            "titulo": "Treinar ensemble ML diário",
            "status": "completed",
            "categoria": "Técnica",
            "desc": "Atualizar modelo com trades recentes",
        },
        {
            "id": "tk_heat",
            "titulo": "Atualizar heatmap setorial",
            "status": "pending",
            "categoria": "Macro",
            "desc": "Recalcular exposição e correlações",
        },
        {
            "id": "tk_rel",
            "titulo": "Gerar relatório parcial",
            "status": "completed",
            "categoria": "Relatórios",
            "desc": "Resumo de execução e rejeições do dia",
        },
        {
            "id": "tk_alert",
            "titulo": "Verificar alertas críticos",
            "status": "pending",
            "categoria": "Operacional",
            "desc": "Circuit breaker / Profit lock / VIX switching",
        },
    ]
...
with col_tv2:
    ...
    pendentes = sum(
        1 for t in st.session_state["task_board"] if t["status"] == "pending"
    )
...
tarefas_exibidas = [
    t
    for t in st.session_state["task_board"]
    if (t["status"] == "pending") or not mostrar_pendentes
]
...
if datetime.now().weekday() == 0:
    st.markdown("---")
    st.markdown(
        '<div class="section-header">🔔 Alertas Semanais</div>', unsafe_allow_html=True
    )
    st.warning(
        "Revisar: Win Rate por classe, distribuição de rejeições por filtro, estabilidade do ML, "
        "limites de subsetor, parâmetros de risco (RR mínimo e risk_per_trade), resultados WFO, "
        "uso de drawdown diário e metas de lucro.",
        icon="⚠️",
    )
    st.caption(
        "Sugestão: ajustar min_rr dinamicamente se WR ≥ 60% e recalibrar filtros em excesso."
    )
    try:
        summary = "WR, PF, Sharpe, rejeições por filtro, subsetor e WFO atualizados."
        import utils as _u

        _u.send_weekly_review_alert(summary)
    except Exception:
        pass
