...
class DailyAnalysisLogger:
    ...
    def __init__(self, log_dir: str = "logs/analysis"):
        ...
        self.strategy_stats = (
            {}
        )  # strategy -> {wins: int, losses: int, ml_confidence_sum: float}
        self.executed_trades = []  # Lista de trades executados para análise
        self._current_bucket = None
        self._cycle_buy_rejections = []
        ...
        if (
            ...
            if not self.current_file.exists():
                with open(self.current_file, "a", encoding="utf-8") as f:
                    ...
                    f.write(
                        f"📅 Janela: {today.strftime('%d/%m/%Y')} {bucket_hour:02d}:00–{(bucket_hour+3)%24:02d}:00\n"
                    )
                    f.write("=" * 80 + "\n\n")
            self.rejections.clear()
            self.strategy_stats.clear()
            self.executed_trades.clear()
            self._cycle_buy_rejections.clear()
            ...
            try:
                ...
                if rejected:
                    ...
                    # Ajuste de Volume Dinâmico no Logger (Sincronizado com Bot)
                        ...
                        0.5 if (12 <= datetime.now().hour < 14) else 0.8
                    ):
                        ...
                        if not score_log.get("RSI_OK") and not score_log.get(
                            "RSI_MODERADO"
                        ):
                        ...
                        if factors:
                            ...
                            detail = " e ".join(
                                factors[:2]
                            )  # Pega os dois primeiros para não ficar gigante
                            if len(factors) > 2:
                                detail += "..."
                            dynamic_reason = f"Configuração de Risco: {detail}"
                        else:
                            dynamic_reason = (
                                f"Score insuficiente para estratégia ({score:.0f})"
                            )
                ...
                if not rejected:
                    ...
                    "aguardando" in reason.lower()
                    or "pullback" in reason.lower()
                    or "correção" in dynamic_reason.lower()
                ):
                    status_emoji = "⏳ AGUARDANDO SETUP"
                    status_line = "-"
                elif (
                    "spread" in reason.lower()
                    or "pedágio" in reason.lower()
                    or spread_pct > 0.1
                ):
                    status_emoji = (
                        "⚠️ LIQUIDEZ BAIXA" if spread_pct > 0.1 else "❌ SPREAD LARGO"
                    )
                    status_line = "-"
                else:
                ...
                log_entry = []
                log_entry.append(
                    f"{timestamp} | INFO | analysis | symbol={symbol} | signal={signal_display} | strategy={strategy_display} | score={score:.0f} | rejected={bool(rejected)} | reason={dynamic_reason}"
                )
                log_entry.append(status_line * 80)
                log_entry.append(
                    f"🕐 {datetime.now().strftime('%H:%M:%S')} | {symbol} | {status_emoji}"
                )
                log_entry.append(status_line * 80)
                ...
                log_entry.append(
                    f"📊 Sinal: {signal_display} | Estratégia: {strategy_display}"
                )
                ...
                is_index = any(
                    x in symbol.upper() for x in ["WIN", "IND", "IBOV", "IFNC"]
                )
                ...
                if rejected and score >= 61:
                    log_entry.append("-" * 40)
                    log_entry.append(
                        f"⚠️  [ALERTA] Sinal Forte detectado, mas ordem não enviada por: {dynamic_reason}"
                    )
                    log_entry.append("-" * 40)
                ...
                if rejected:
                    if symbol not in self.rejections:
                        self.rejections[symbol] = {
                            "reasons": {},
                            "last_time": timestamp,
                            "score": score,
                        }
                    r_info = self.rejections[symbol]
                    r_info["reasons"][dynamic_reason] = (
                        r_info["reasons"].get(dynamic_reason, 0) + 1
                    )
                    r_info["last_time"] = timestamp
                    r_info["score"] = score
                    if (signal or "").upper() == "BUY":
                        self._cycle_buy_rejections.append(
                            {
                                "timestamp": timestamp,
                                "symbol": symbol,
    ...
    def get_cycle_buy_rejections_report(self) -> str:
        with self.lock:
            today = datetime.now().date()
            bucket_hour = (datetime.now().hour // 3) * 3
            header = [
                "=" * 80,
                f"🧾 CICLO {bucket_hour:02d}:00–{(bucket_hour+3)%24:02d}:00 | {today.strftime('%d/%m/%Y')}",
                "=" * 80,
            ]
            entries = sorted(
                self._cycle_buy_rejections, key=lambda x: (x["symbol"], x["timestamp"])
            )
            lines = []
            reason_counts = {}
            for e in entries:
                ...
                ind = e["indicators"] or {}
                score_log = ind.get("score_log", {}) or {}
                missing = []
                try:
                    import config

                    min_score = getattr(config, "MIN_SIGNAL_SCORE", 60)
                except Exception:
                    min_score = 60
                if e["score"] < float(min_score):
                    missing.append("Score mínimo")
                if float(ind.get("adx", 0) or 0) < 20:
                    missing.append("ADX ≥ 20")
                if float(ind.get("rsi", 0) or 0) > 70:
                    missing.append("RSI ≤ 70")
                vr = float(ind.get("volume_ratio", 0) or 0)
                hr = datetime.now().hour
                vr_min = 0.5 if (12 <= hr < 14) else 0.8
                if vr < vr_min:
                    missing.append(f"Volume ≥ {vr_min:.2f}x")
                if float(ind.get("spread_pct", 0) or 0) > 0.1:
                    missing.append("Spread ≤ 0.10%")
                if (ind.get("ema_trend") or "N/A") != "UP":
                    missing.append("Tendência EMA UP")
                if not score_log.get("MACD_CROSS"):
                    missing.append("Cruzamento MACD")
                if not score_log.get("VOL_BOOST"):
                    missing.append("Boost de Volume")
                if not score_log.get("MOMENTUM"):
                    missing.append("Momentum")
                if not score_log.get("RSI_OK") and not score_log.get("RSI_MODERADO"):
                    missing.append("RSI na zona ideal")
                detail = (
                    " | ".join(missing)
                    if missing
                    else "Critérios atendidos, aguardando gatilho"
                )
                lines.append(
                    f"🕐 {e['timestamp']} | {e['symbol']} | Motivo: {e['reason']} | Falta: {detail}"
                )
            stats = ["-" * 80, "📊 Estatísticas por motivo:"]
            for r, c in sorted(reason_counts.items(), key=lambda x: x[1], reverse=True):
                stats.append(f"• {r}: {c}x")
            return "\n".join(
                header
                + (lines if lines else ["Nenhuma rejeição de compra no ciclo"])
                + stats
            )
    ...
    def get_daily_rejection_summary(self) -> str:
        ...
        with self.lock:
            ...
            sorted_rejections = sorted(
                self.rejections.items(), key=lambda x: x[1]["score"], reverse=True
            )
    ...
    def log_trade_result(
        ...
        with self.lock:
            ...
            if ml_confidence > 0:
                ...
                stats["ml_total"] += 1
                if (ml_prediction == "BUY" and pnl > 0) or (
    ...
    def get_strategy_win_rates(self) -> str:
        ...
        with self.lock:
            ...
            for strategy, stats in self.strategy_stats.items():
                ...
                lines.append(
                    f"{emoji} {strategy:15} | WR: {wr:5.1f}% | W:{stats['wins']} L:{stats['losses']}{ml_acc}"
                )
...
def log_signal_analysis_to_file(
    symbol: str,
    signal: str,
    strategy: str,
    score: float,
