...
if rates_large is not None and len(rates_large) > 0:
    print(
        f"   ✅ {len(rates_large)} barras em {elapsed:.2f}s ({len(rates_large)/elapsed:.0f} barras/s)"
    )
...
if all_ok:
    ...
    checks["MT5 Conectado"] and checks["Símbolos Válidos"] and checks["Dados Carregam"]
):
