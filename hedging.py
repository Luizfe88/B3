...
class PredictiveHedger:
    ...
    def predict_risk_trend(self) -> str:
        ...
        older = (
            [r["score"] for r in self.risk_history[-20:-10]]
...
def check_unwind_trigger(current_dd: float, vix_br: float):
    ...
    if (
        current_dd < config.HEDGE_UNWIND_DD_THRESHOLD
        or vix_br < config.HEDGE_UNWIND_VIX_THRESHOLD
    ):
        ...
        for pos in positions:
            if "Hedge" in pos.comment:
                logger.warning(
                    f"🏁 Trigger Unwind: DD={current_dd:.2%}, VIX={vix_br}. Desfazendo {pos.symbol}"
                )
...
def hedge_with_options(symbol: str):
    ...
    try:
        ...
        puts = [
            ...
            if len(s.name) >= 7 and s.name[4] in "MNOPQRSTUVWX"
        ]
        ...
        option_volume = max(
            100,
            round(predictive_hedger.calculate_hedge_size(risk["score"]) / 100) * 100,
        )
