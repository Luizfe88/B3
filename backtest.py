...
def simulate_strategy(
    symbol, days=30, ema_short=9, ema_long=21, rsi_low=30, rsi_high=70, df=None
):
    """
    Simula estratégia de cruzamento de EMA + RSI

    Args:
        symbol: Ativo a simular
        days: Dias de backtest
        ema_short: Média móvel curta
        ema_long: Média móvel longa
    ...
    # Simula sinais
    df["signal"] = 0
    df.loc[(df["ema_short"] > df["ema_long"]) & (df["rsi"] < rsi_high), "signal"] = (
        1  # BUY
    )
    df.loc[(df["ema_short"] < df["ema_long"]) & (df["rsi"] > rsi_low), "signal"] = (
        -1
    )  # SELL
                ...
                trades.append(
                    {
                        "pnl": pnl,
                        "side": "BUY",
                        "duration": row["time"] - position["time"],
                    }
                )
                position = None
    ...
    logger.info(
        f"📊 Simulação {symbol}: WR={win_rate:.1f}%, Trades={total}, PnL={total_pnl:.2f}"
    )
    ...
    return (
        True,
        f"Parâmetros válidos (WR: {results['win_rate']:.1f}%, R:R: {results['rr_ratio']:.2f})",
    )
...
def run_walk_forward(symbol: str, days: int = 60, segments: int = 4) -> List[Dict]:
    ...
    for i in range(segments):
        ...
        if "error" not in oos_perf:
            ...
            logger.info(
                f"✅ WFO Segmento {i+1}: Train WR={best_wr:.1f}% | Test WR={oos_perf['win_rate']:.1f}%"
            )
